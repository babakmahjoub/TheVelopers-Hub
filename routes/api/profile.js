...
router.get('/', async (req, res) => {
  try {
    const profiles = await Profile.find().populate('user', ['name', 'avatar']); //profile.user is now an array of instances of 'User'
    res.json(profiles);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.get('/user/:user_id', async (req, res) => {
  try {
    ...
    }).populate('user', ['name', 'avatar']); //profile.user is now an array of instances of 'User'

    if (!profile) return res.status(400).json({ msg: 'There is no such profile' });
